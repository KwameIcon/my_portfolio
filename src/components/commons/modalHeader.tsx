import { removeTaskbarIcon, toggleMaximized, toggleMinimized } from "@/store/slice/taskbarSlice";
    ...
    return(
        <div className="absolute top-0 left-0 w-full h-10 bg-[#0F4774] flex items-center justify-center">
            ...
            <ul className={`absolute right-0 h-full w-fit px-5 flex items-center justify-center gap-3 group ${isMinimized ? "hidden" : ''}`}>
                ...
                <li className="w-5 h-5 flex items-center justify-center cursor-pointer rounded-full bg-red-400" onClick={() => dispatch(removeTaskbarIcon({ id: id }))}><X size={12} className="group-hover:block hidden"/></li>
            </ul>
        </div>
    )
}